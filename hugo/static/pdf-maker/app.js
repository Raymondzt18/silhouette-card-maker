...
// Riftmana's search API (used to generate the bundled snapshot below,
// server-side) doesn't send an Access-Control-Allow-Origin header, so a
// cross-origin browser fetch() to it is blocked by CORS even though the same
// request works fine server-side (e.g. from api.py's fetch_card_number, or
// curl). To resolve names without depending on that, this page ships a
// static name -> card number snapshot (generated by
// plugins/riftbound/generate_card_index.py, re-run periodically as new sets
// release) and fetches it same-origin, which isn't subject to CORS at all.
const CARD_INDEX_URL = "riftbound-card-index.json";
const cardIndexPromise = fetch(versioned(CARD_INDEX_URL))
  .then((res) => (res.ok ? res.json() : {}))
  .catch(() => ({}));

// Riftcodex (https://riftcodex.com), a separate community Riftbound API, does
// send Access-Control-Allow-Origin: * — unlike Riftmana, it's actually
// reachable live from a browser. It has no server-side name filter, so a
// lookup means paginating its full catalog (~15 requests at its 100/page
// max); that's cheap once and is cached for the rest of the session, used
// only as a fallback for names the bundled snapshot above doesn't have yet
// (e.g. a card from a set released after the snapshot was last regenerated).
const RIFTCODEX_API_URL = "https://api.riftcodex.com/cards";
const RIFTCODEX_PAGE_SIZE = 100;
// Promotional reprints (organized-play, alt finishes like "Metal") aren't
// reliably flagged by Riftcodex's alternate_art/overnumbered/signature
// metadata, so this set is excluded outright when picking a default printing.
const RIFTCODEX_PROMO_SET_ID = "OPP";

let riftcodexIndexPromise = null;

function loadRiftcodexIndex() {
  if (!riftcodexIndexPromise) {
    riftcodexIndexPromise = (async () => {
      const cards = [];
      let page = 1;
      let pages = 1;
      do {
        const res = await fetch(`${RIFTCODEX_API_URL}?page=${page}&size=${RIFTCODEX_PAGE_SIZE}`, {
          headers: { Accept: "application/json" },
        });
        if (!res.ok) throw new Error(`riftcodex lookup failed (HTTP ${res.status})`);
        const json = await res.json();
        cards.push(...(json.items || []));
        pages = json.pages || 1;
        page += 1;
      } while (page <= pages);
      ...
      for (const card of cards) {
        ...
        if (!name) continue;
        // Variant printings suffix the base name with "(Signature)",
        // "(Alternate Art)", "(Metal)", etc. — group all printings of a card
        // under that shared base name.
        const baseName = name.replace(/\s*\([^)]*\)\s*$/, "").trim().toLowerCase();
        if (!byBaseName.has(baseName)) byBaseName.set(baseName, []);
        byBaseName.get(baseName).push(card);
      }

      const index = {};
      for (const [baseName, variants] of byBaseName) {
        // Prefer a printing whose raw name has no "(...)" suffix at all (the
        // true default art) from a non-promo set; fall back to whatever's
        // available if every printing is a promo/alternate.
        const clean = variants.filter(
          (card) => (card.name || "").trim().toLowerCase() === baseName && card.set?.set_id !== RIFTCODEX_PROMO_SET_ID
        );
        const pool = clean.length > 0 ? clean : variants;
        const pick = pool.slice().sort((a, b) => (a.collector_number || 0) - (b.collector_number || 0))[0];
        if (pick?.set?.set_id && pick?.collector_number != null) {
          index[baseName] = `${pick.set.set_id}-${String(pick.collector_number).padStart(3, "0")}`;
        }
      }
      return index;
    })();
  }
  return riftcodexIndexPromise;
}

// Resolves a card name to its card number. Checks the bundled offline
// snapshot first (instant, no network); falls back to a live Riftcodex
// lookup for names not yet in the snapshot.
async function resolveCardNumber(name) {
  const normalizedName = name.trim().toLowerCase();

  const index = await cardIndexPromise;
  if (index[normalizedName]) return normalizeCardNumber(index[normalizedName]);

  const riftcodexIndex = await loadRiftcodexIndex();
  if (riftcodexIndex[normalizedName]) return normalizeCardNumber(riftcodexIndex[normalizedName]);
